...
export class TruyenFull2Plugin implements ICrawl, Plugin {
	id = 2;
	name = 'truyenfull2';
	...
	async getDetails(name: string, page: number): Promise<Details> {
		...
		$('.a[itemprop="genre"]').each((_, el) => {
			const genre = new Genre($(el).text().trim(), $(el).attr('href')!, ''); // Add an empty string as the third argument
	...
	async getGenres(): Promise<Genre[]> {
		...
		$('ul.control.nav.navbar-nav > li.dropdown')
			...
			.each((_, el) => {
				...
				genres.push(new Genre(id!, href!, name));
			});
