...
export const getFullBookContent = async (id, downSource) => {
    ...
    try {
        const bookResponse = await axios.get(`${apiURL}/novels?sources=${downSource}&name=${id}&page=1`);
        const { title , cover, author, gneres, source, status, description, chapters, maxPage } = bookResponse.data;
        ...
        const chapterPromises = chapters.map((chapter, index) => getChapter(id, index+1, downSource));
...
export const getUpdateBook = async (oldBook, lastChap, source) => {
    try{
        ...
        return {
            ...oldBook,
            chapters: oldBook.chapters.concat([...Array(lastChap - oldBook.chapters.length).keys()]),
