import { text_font, text_size, line_height, 
    setFont, setFontSize, setBackground, setTextColor, setLineHeight, 
...
const SettingBox = () => {
  ...
  const [lineHeightState, setLineHeightState] = useState(getLineHeight());
    ...
    return (
      <div className="fixed left-20 top-1/2 transform -translate-y-1/2 bg-white p-4 rounded-lg shadow-lg w-64">
        <h3 className="font-bold mb-4">Cài đặt</h3>
        <div className="mb-4">
            <label className="block mb-2">Font chữ</label>
            <select
              className="w-full border border-gray-300 p-2"
              onChange={(e) => {setFont(e.target.value); setFontFamilyState(e.target.value)}}
              value={fontState}
            >
          ...
          <div className="mb-4">
            ...
            <select
              className="w-full border border-gray-300 p-2"
              onChange={(e) => {setFontSize(e.target.value); setFontSizeState(e.target.value)}}
              value={fontSizeState}
            >
              {text_size.map((size, index) => (
                <option key={index} value={size.value}>{size.name}</option>
              ))}
            </select>
          </div>
          <div className="mb-4">
            ...
            <select
              className="w-full border border-gray-300 p-2"
              onChange={(e) => {setLineHeight(e.target.value); setLineHeightState(e.target.value)}}
              value={lineHeightState}
            >
              {line_height.map((height, index) => (
                <option key={index} value={height.value}>{height.name}</option>
              ))}
            </select>
